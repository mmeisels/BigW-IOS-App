angular.module('nibs_ibeacon.offer', ['openfb', 'nibs_ibeacon.status', 'nibs_ibeacon.activity', 'nibs_ibeacon.wallet'])
    ...
    .config(function ($stateProvider) {
        ...
        $stateProvider
            ...
            .state('app.offer-thanks', {
                url: "/offers/thanks",
                views: {
                    'menuContent' :{
                        templateUrl: "templates/offer-thanks.html"
                    }
                }
            })
            
            .state('app.offer-qrcode', {
                url: "/offers/qrcode",
                views: {
                   'menuContent' :{
                        templateUrl: "templates/offer-qrcode.html"
                    }
                }
            })
...
//                });
        ...
        $scope.drone = function () {
            Status.show('Delivery Drone Ordered');
            Activity.create({type: "Drone Delivery", points: 1000, offerId: $scope.offer.sfid, name: $scope.offer.name, image: $scope.offer.image})
                .success(function(status) {
                    Status.checkStatus(status);
                });
            $state.go('app.offer', {offerId: $scope.offer.id});
        };
